...
--- Recursively scans a directory path to build a tree of bsi.Node objects.
---@param path string
---@param depth integer
---@param opts table|nil { expand_all, show_ignored, max_depth }
---@return bsi.Node|nil
function Provider:scan(path, depth, opts)
  ...
  for name in pairs(children_map) do
    ...
    if is_dir then
      ...
      if at_max_depth and not opts.expand_all then
        child = {
          id = fullpath,
          name = name,
          path = fullpath,
  ...
  -- Collapse single-child directory chains (foo/bar/baz -> foo/bar/baz)
